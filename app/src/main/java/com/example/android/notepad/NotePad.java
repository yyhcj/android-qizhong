...
/**
 定义了笔记应用内容提供者（Note Pad content provider）与其客户端之间的契约。
 契约将客户端访问内容提供者所需的信息定义为一个或多个数据表。
 契约是一个公开的、不可继承（最终的，即 final 修饰）的类，它包含用于定义列名和统一资源标识符（URIs）的常量。一个编写良好的客户端仅依赖于此契约中的常量。
 */
