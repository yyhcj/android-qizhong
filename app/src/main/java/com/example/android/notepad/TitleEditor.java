...
/**
 此活动允许用户编辑笔记的标题。它会显示一个包含编辑文本框（EditText）的悬浮窗口。
 注意：请注意，此活动中的内容提供者（Content Provider）操作是在用户界面（UI）线程上进行的。这并非一种良好的做法。在此处这样做只是为了使代码更具可读性。在实际的应用程序中，应当使用 {@link android.content.AsyncQueryHandler}（异步查询处理器）
 或者 {@link android.os.AsyncTask}（异步任务）对象，以便在单独的线程上异步执行操作。
 */
